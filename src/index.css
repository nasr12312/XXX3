...
export default function App() {
    ...
    const [view, setView] = useState({ page: 'login' }); // login, register, studentEntry, examTaking
    ...
    if (view.page === 'examEntry') {
        return <StudentExamEntryScreen examId={view.examId} />;
    }
    ...
    if (!user || !userData) {
        return (
            <div className="bg-gray-50 dark:bg-gray-900">
                {view.page === 'login' ? (
                    <LoginScreen onSwitchToRegister={() => setView({ page: 'register' })} />
                ) : (
                    <RegisterScreen onSwitchToLogin={() => setView({ page: 'login' })} />
                )}
            </div>
        );
    }
                        ...
                        <div className="mt-4 pt-4 border-t flex flex-col md:flex-row md:items-center md:space-x-4" dir="ltr">
                             <div className="flex-1 mb-2 md:mb-0">
                                ...
                                <div className="flex items-center">
                                    <input type="text" readOnly value={`${window.location.origin}${window.location.pathname}#${`/exam/${exam.id}`}`} className="w-full p-2 border rounded-l-md bg-gray-50 text-sm"/>
                                    <button onClick={() => copyToClipboard(`${window.location.origin}${window.location.pathname}#${`/exam/${exam.id}`}`)} className="bg-gray-200 px-3 py-2 border border-l-0 rounded-r-md hover:bg-gray-300"><Copy size={16}/></button>
                                </div>
                            </div>
...
function StudentExamEntryScreen({ examId }) {
    ...
    useEffect(() => {
        const fetchExam = async () => {
            if (!examId) {
                setError("معرف الامتحان غير موجود.");
                setLoading(false);
                return;
            }
            ...
            if (docSnap.exists()) {
                ...
                setError("لم يتم العثور على هذا الامتحان.");
            }
            setLoading(false);
        };
        fetchExam();
    ...
    const handleStartExam = async (e) => {
        ...
        if (exam.examCode !== examCode) {
            setError("رمز الامتحان غير صحيح.");
            return;
        }

        const q = query(collection(db, "students"), where("studentCode", "==", studentCode), where("classId", "==", exam.classId));
        const studentSnap = await getDocs(q);
        if (studentSnap.empty) {
            setError("رمز الطالب غير صحيح أو لا ينتمي إلى شعبة هذا الامتحان.");
            return;
        }
    ...
    if (loading) {
        return <div className="flex items-center justify-center h-screen bg-gray-100"><div className="text-xl font-semibold">جاري تحميل الامتحان...</div></div>;
    }

    if (!exam) {
        return <div className="flex items-center justify-center h-screen bg-red-100 text-red-700">{error || "لم يتم العثور على الامتحان."}</div>;
    }
    
    if (student) {
        return <ExamTakingScreen exam={exam} student={student} />;
    }
    ...
    return (
        <div className="flex items-center justify-center h-screen bg-gray-100">
            <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-lg">
                ...
                <p className="text-center text-gray-500 mb-6">مرحباً بك. الرجاء إدخال الرموز لبدء الامتحان.</p>
                <form onSubmit={handleStartExam} className="space-y-6">
                    <div>
                        <label htmlFor="studentCode" className="block text-sm font-medium text-gray-700">رمز الطالب الخاص بك</label>
                        <input type="text" id="studentCode" value={studentCode} onChange={(e) => setStudentCode(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required />
                    </div>
                    <div>
                        <label htmlFor="examCode" className="block text-sm font-medium text-gray-700">رمز الامتحان</label>
                        <input type="text" id="examCode" value={examCode} onChange={(e) => setExamCode(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required />
                    </div>
...
// Exam Taking Screen for Students
function ExamTakingScreen({ exam, student }) {
    ...
    if (submitted) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-gray-100 text-center p-4">
                <div className="bg-white p-10 rounded-lg shadow-xl">
                    <h1 className="text-3xl font-bold text-gray-800 mb-4">تم تسليم إجاباتك بنجاح!</h1>
                    <p className="text-gray-600 mb-6">نتيجتك هي: <span className="font-bold text-2xl text-blue-600">{finalScore} / {exam.questions.length}</span></p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="bg-gray-100 min-h-screen p-8">
            <div className="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-lg">
                ...
                <p className="text-gray-600 mb-6">مرحباً, {student.name}. بالتوفيق!</p>
                <div className="space-y-8">
                        ...
                        <div key={qIndex} className="border-t pt-6">
                            ...
                            <div className="space-y-3">
                                {q.options.map((option, oIndex) => (
                                    <label key={oIndex} className="flex items-center p-3 rounded-lg border hover:bg-gray-50 cursor-pointer">
                                        <input 
                                            type="radio" 
                ...
                <div className="mt-8 flex justify-center">
                    <button onClick={handleSubmitExam} className="px-10 py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700">
                        تسليم الإجابات
                    </button>
                </div>
            </div>
